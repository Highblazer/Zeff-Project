...
from lib.trading_safety import check_kill_switch, validate_price, pre_trade_checks, validate_rr, MIN_RR_RATIO
...
# ── Pair configuration ──
# Ali.bot: Tight stops, 1:3 R:R — trades resolve in 1-5 days
# Fewer pairs than TradeBot — focused on liquid, high-conviction instruments
PAIRS = {
    # ── Forex Majors — 20-30 pip SL, 60-90 pip TP (1:3 R:R) ──
    # Volumes target ~4% of balance ($18) risk per trade.
    'EURUSD':  {'name': 'EUR/USD',  'type': 'forex',  'volume': 700000,  'risk_pips': 25,  'reward_pips': 75,  'trail_trigger_pips': 25,  'trail_step_pips': 12, 'est_spread_pips': 1},
    'GBPUSD':  {'name': 'GBP/USD',  'type': 'forex',  'volume': 600000,  'risk_pips': 30,  'reward_pips': 90,  'trail_trigger_pips': 30,  'trail_step_pips': 15, 'est_spread_pips': 2},
    'USDJPY':  {'name': 'USD/JPY',  'type': 'forex',  'volume': 900000,  'risk_pips': 30,  'reward_pips': 90,  'trail_trigger_pips': 30,  'trail_step_pips': 15, 'est_spread_pips': 2},
    'AUDUSD':  {'name': 'AUD/USD',  'type': 'forex',  'volume': 700000,  'risk_pips': 25,  'reward_pips': 75,  'trail_trigger_pips': 25,  'trail_step_pips': 12, 'est_spread_pips': 2},
    'USDCAD':  {'name': 'USD/CAD',  'type': 'forex',  'volume': 600000,  'risk_pips': 30,  'reward_pips': 90,  'trail_trigger_pips': 30,  'trail_step_pips': 15, 'est_spread_pips': 2},
    'NZDUSD':  {'name': 'NZD/USD',  'type': 'forex',  'volume': 900000,  'risk_pips': 20,  'reward_pips': 60,  'trail_trigger_pips': 20,  'trail_step_pips': 10, 'est_spread_pips': 2},
    # ── Forex Crosses — slightly wider for higher volatility ──
    'EURJPY':  {'name': 'EUR/JPY',  'type': 'forex',  'volume': 700000,  'risk_pips': 40,  'reward_pips': 120, 'trail_trigger_pips': 40,  'trail_step_pips': 20, 'est_spread_pips': 3},
    'GBPJPY':  {'name': 'GBP/JPY',  'type': 'forex',  'volume': 500000,  'risk_pips': 50,  'reward_pips': 150, 'trail_trigger_pips': 50,  'trail_step_pips': 25, 'est_spread_pips': 4},
    # ── Crypto ──
    'BTCUSD':  {'name': 'BTC/USD',  'type': 'crypto', 'volume': 100,     'risk_pips': 800,  'reward_pips': 2400,  'trail_trigger_pips': 800,  'trail_step_pips': 400, 'est_spread_pips': 50},
    'ETHUSD':  {'name': 'ETH/USD',  'type': 'crypto', 'volume': 1300,    'risk_pips': 60,   'reward_pips': 180,   'trail_trigger_pips': 60,   'trail_step_pips': 30, 'est_spread_pips': 5},
    # ── Commodities ──
    'XAUUSD':  {'name': 'Gold',     'type': 'commodity', 'volume': 400,  'risk_pips': 200,  'reward_pips': 600,   'trail_trigger_pips': 200,  'trail_step_pips': 100, 'est_spread_pips': 30},
    # ── Indices ── (pip_val = 0.1 for these)
    'US500':   {'name': 'S&P 500',  'type': 'index',  'volume': 100,     'risk_pips': 80,   'reward_pips': 240,   'trail_trigger_pips': 80,   'trail_step_pips': 40, 'est_spread_pips': 5},
    'USTEC':   {'name': 'Nasdaq',   'type': 'index',  'volume': 100,     'risk_pips': 120,  'reward_pips': 360,   'trail_trigger_pips': 120,  'trail_step_pips': 60, 'est_spread_pips': 10},
}
...
def _pip_value(symbol, asset_type):
...
def analyze_layer_6_risk_reward(symbol, direction, current_price, daily_data, config):
    ...
    pip_value = _pip_value(symbol, asset_type)
...
class AliBotEngine:
    def __init__(self):
        ...
        self._skip_symbols = {}  # symbol -> datetime when cooldown expires
        ...
        try:
            ...
            if hasattr(payload, 'trader'):
                ...
                if self._starting_balance == 0.0:
                    self._starting_balance = self.balance
    ...
    def _on_reconcile(self, msg):
        ...
        for pos in getattr(payload, 'position', []):
            ...
            config = PAIRS.get(symbol, {})
            pip_val = _pip_value(symbol, config.get('type', 'forex'))
            if broker_trailing:
                phase = 'trailing'
            elif sl and entry:
        ...
        if msg.payloadType == 2126:  # ProtoOAExecutionEvent
                    ...
                    if etype == 2 and vol > 0:  # ORDER_FILLED with volume > 0
                        ...
                        if existing and vol < existing.get('volume', 0):
                            ...
                            print(f"  [SCALE-OUT FILL] {sym} posId={pid} partial close confirmed: "
                                  f"{closed_lots:.2f}lot closed, {lots:.2f}lot remaining")
                            existing['volume'] = vol
                            ...
                            if pending and (not sl_price or not tp_price):
                                ...
                                pip_val = _pip_value(sym, asset_type)
                            ...
                            # LAYER 2 SL-GUARD: If SL is missing on broker, force-amend it
            ...
            try:
                ...
                if pid in self.trailing_state:
                    ...
                    # Remove TP once broker-trailed SL locks >= trigger_pips from entry
                    if not state.get('tp_removed', False):
                        config = PAIRS.get(sym, {})
                        trigger_pips = config.get('trail_trigger_pips', config.get('risk_pips', 0))
                        entry = state['entry_price']
                        asset_type = config.get('type', 'forex')
                        pv = _pip_value(sym, asset_type)
                        sl_locked = abs(new_sl - entry) / pv if pv else 0
                        if sl_locked >= trigger_pips and trigger_pips > 0:
                            print(f"  [TRAIL-TP-REMOVED] {sym} SL locks {sl_locked:.1f} pips >= {trigger_pips} trigger — removing TP")
                            state['tp_removed'] = True
                            self._amend_sl(pid, sym, new_sl, trailing=True, clear_tp=True)
    ...
    def _cycle_reconcile(self):
        """Send reconcile request to broker at the start of every cycle.
        Prunes ghost positions from local state and adds any missing broker positions.
        Returns a Deferred that fires when reconciliation is complete."""
        ...
        d.addErrback(self._on_reconcile_error)
        return d
    ...
    def _process_cycle_reconcile(self, msg):
        ...
        payload = Protobuf.extract(msg)
        if hasattr(payload, 'errorCode'):
            print(f"  [RECONCILE] Error: {payload.errorCode} — continuing with cached state")
            return

        # Build set of position IDs that the broker actually has open
        broker_pids = set()
        broker_positions = {}  # pid -> position data
        for pos in getattr(payload, 'position', []):
            pid = pos.positionId
            broker_pids.add(pid)
            sym_id = pos.tradeData.symbolId
            ...
            broker_positions[pid] = {
                ...
                'bot': 'alibot' if is_mine else 'tradebot',
                'entry': pos.price,
                'sl': pos.stopLoss if pos.stopLoss else 0,
                'tp': pos.takeProfit if pos.takeProfit else 0,
            }
        ...
        # Also prune local_positions that have no matching entry in self.positions
        orphan_syms = []
        local_pid_syms = {p['symbol'] for p in self.positions.values()}
        for sym in list(self.local_positions.keys()):
            if sym not in local_pid_syms:
                # Check if broker has this symbol under a different pid
                broker_has = any(bp['symbol'] == sym for bp in broker_positions.values())
                if not broker_has:
        ...
        if missing_pids:
            for pid in missing_pids:
                ...
                self.positions[pid] = {
                    ...
                    'volume': bp['volume'], 'positionId': pid, 'bot': 'alibot',
                }
                self.local_positions[sym] = {
        ...
        if changes:
            ...
            print(f"  [RECONCILE] OK — {len(self.positions)} positions in sync")
        ...
        for pid, bp in broker_positions.items():
            ...
            if bp['sl'] == 0 or bp['tp'] == 0:
                ...
                ep = bp['entry']
                asset_type = config.get('type', 'forex')
                pip_value = _pip_value(sym, asset_type)
    ...
    def _on_reconcile_error(self, failure):
        print(f"  [RECONCILE] Request failed: {failure.getErrorMessage()} — continuing with cached state")
    ...
    def _trading_cycle(self):
        ...
        # Reconcile with broker FIRST — broker is the single source of truth
        d = self._cycle_reconcile()
        d.addCallback(lambda _: self._run_analysis_cycle())
        d.addErrback(lambda f: print(f"[{self._ts()}] Cycle error: {f.getErrorMessage()}"))
        return d

    def _run_analysis_cycle(self):
        """The actual analysis logic, called after reconciliation completes."""
        ...
        def _fetch_price(sym):
            return sym, get_current_price(sym)

        all_prices = {}
        with ThreadPoolExecutor(max_workers=6) as pool:
            futures = {pool.submit(_fetch_price, s): s for s in PAIRS}
            for future in as_completed(futures):
                try:
                    sym, price = future.result(timeout=15)
                    if price:
        ...
        for symbol, config, analysis in tradeable_setups:
            ...
            self._execute_order(symbol, direction, price, config,
                               f"{analysis['score']}/6|{layer_summary[:50]}",
                               alert_text=(
        ...
        for pid, state in list(self.trailing_state.items()):
            ...
            if phase == 'initial' and profit_pips >= trigger_pips:
                ...
                if direction == 'BUY':
                    ...
                    new_sl = round(entry - step_offset, 5)
                # TP kept until SL locks in >= 1x risk (trigger_pips) of profit
                ...
                print(f"  [TRAIL] {symbol} -> TRAILING ACTIVE (profit {profit_pips:.1f} pips, {tp_status})")
    ...
    def _execute_order(self, symbol, direction, entry_price, config, reason, alert_text=None):
        if symbol in self._skip_symbols:
            if datetime.now() < self._skip_symbols[symbol]:
                print(f"  >> SKIPPING {symbol} — cooldown until {self._skip_symbols[symbol].strftime('%H:%M:%S')}")
                return
            else:
                del self._skip_symbols[symbol]  # Cooldown expired — retry
        sym_id = SYMBOL_IDS.get(symbol)
        if not sym_id:
            return
        ...
        # pip_multiplier = pip_value / broker_point_size (confirmed from fill prices)
        if asset_type == 'forex':
            pip_multiplier = 10
        elif symbol == 'XAGUSD':
            pip_multiplier = 10  # Silver: 3 decimals
        else:
            pip_multiplier = 1   # All other CFDs
        ...
        print(f"  >> SENDING {direction} {symbol} {lot_display:.2f}lot SL={config['risk_pips']}pip TP={config['reward_pips']}pip "
              f"(pts={risk_points}/{reward_points})")

        d = self.client.send(req, responseTimeoutInSeconds=30)
        config_with_reason = dict(config, _reason=reason)
        d.addCallback(lambda msg, s=symbol, d2=direction, ep=entry_price, c=config_with_reason, at=alert_text: self._on_order_response(msg, s, d2, ep, c, at))
        d.addErrback(lambda f, s=symbol: self._on_order_error(f, s))

    def _on_order_response(self, msg, symbol, direction, entry_price, config, alert_text=None):
        ...
        # Only alert after broker accepts the order
        if alert_text:
    ...
    def _on_order_error(self, failure, symbol):
        ...
        # Temporary 5-minute cooldown instead of permanent ban
        self._skip_symbols[symbol] = datetime.now() + timedelta(minutes=5)
        print(f"  >> {symbol} cooldown for 5 minutes (retry after {self._skip_symbols[symbol].strftime('%H:%M:%S')})")
    ...
    def _get_pip_value(symbol, asset_type):
        """Price distance per pip — centralized for consistency."""
        if 'JPY' in symbol and asset_type == 'forex':
            return 0.01
        elif asset_type == 'forex':
            return 0.0001
    ...
    def _watchdog(self):
        """Save state every 30s and force-restart if disconnected too long."""
        self._save_state()
        if self.authenticated:
            return
        ...
        if down_seconds > 180:
            ...
            self._alert(f"WATCHDOG: Ali.bot disconnected {down_seconds:.0f}s — force-restarting")
            try:
